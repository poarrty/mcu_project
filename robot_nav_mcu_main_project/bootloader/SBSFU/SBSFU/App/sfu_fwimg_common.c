...
SFU_IMG_InitStatusTypeDef SFU_IMG_InitImageHandling(void)
{
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
...
#endif /* (__GNUC__) */
    {
      if (!IS_ALIGNED(SlotStartAdd[SLOT_ACTIVE_1 + i]))
      {
        ...
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%ld (%lx) is not properly aligned\r\n",
  ...
  for (i = 0U; i < SFU_NB_MAX_DWL_AREA; i++)
  {
...
#endif /* (__GNUC__) */
    {
      ...
      {
        e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
        TRACE("\r\n= [FWIMG] SLOT_DWL_%ld (%lx) is not properly aligned\r\n",
  ...
  {
    e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
    TRACE("\r\n= [FWIMG] magic size (%ld) is not matching the FLASH constraints\r\n", MAGIC_LENGTH);
  ...
  if (0U != (uint32_t)(SFU_IMG_CHUNK_SIZE % (uint32_t)sizeof(SFU_LL_FLASH_write_t)))
  {
    ...
    e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
    TRACE("\r\n= [FWIMG] Decrypt chunk size (%ld) is not matching the FLASH constraints\r\n", SFU_IMG_CHUNK_SIZE);
  ...
  if (0U != (uint32_t)((uint32_t)SFU_IMG_CHUNK_SIZE % (uint32_t)AES_BLOCK_SIZE))
  {
    ...
    TRACE("\r\n= [FWIMG] Chunk size (%ld) is not matching the AES CBC constraints\r\n", SFU_IMG_CHUNK_SIZE);
  }
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
...
#endif /* (__GNUC__) */
    {
      ...
      {
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%ld overlaps SBSFU code area protected by WRP\r\n", i + 1U);
        e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
      }
    }
  }
  ...
  for (i = 0U; i < SFU_NB_MAX_DWL_AREA; i++)
  {
...
#endif /* (__GNUC__) */
    {
      ...
      {
        TRACE("\r\n= [FWIMG] SLOT_DWL_%ld overlaps SBSFU code area protected by WRP\r\n", i + 1U);
        e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
      }
    }
  }
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
...
#endif /* (__GNUC__) */
    {
      ...
      {
        TRACE("\r\n= [FWIMG] Header of SLOT_ACTIVE_%ld is not under the MPU protection\r\n", i + 1U);
        e_ret_status = SFU_IMG_INIT_FLASH_CONSTRAINTS_ERROR;
      }
    }
  }
...
SFU_ErrorStatus SFU_IMG_VerifyActiveImg(uint32_t SlotNumber)
{
  ...
  if (SFU_ERROR == e_ret_status)
  {
    ...
    TRACE("\r\n=         SFU_IMG_VerifyActiveImg failure with se_status=%ld!", e_se_status);
  }
