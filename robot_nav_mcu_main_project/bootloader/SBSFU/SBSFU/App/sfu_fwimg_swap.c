...
static const int8_t SWAPPED[sizeof(SFU_LL_FLASH_write_t)]  = {0};
static const int8_t NOT_SWAPPED[sizeof(SFU_LL_FLASH_write_t)] = {-1};
...
static SFU_ErrorStatus FirmwareToRollback(uint32_t SlotNumber, SE_FwStateTypeDef MasterSlotState)
{
  ...
  if (SFU_SUCCESS == SFU_IMG_DetectFW(SlotNumber))
  {
    ...
    if (e_ret_status_se == SE_SUCCESS)
    {
      ...
      TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%ld state = %ld", SlotNumber, image_state);
...
#if defined(SFU_VERBOSE_DEBUG_MODE)
  TRACE("\r\n\t  %ld bytes of ciphertext decrypted.", fw_decrypted_total_size);
#endif /* SFU_VERBOSE_DEBUG_MODE */
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CheckSwapImageHandling(void)
{
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
    if (SlotStartAdd[SLOT_ACTIVE_1 + i] != 0U)
    {
      ...
      {
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%ld size (%ld) must be a multiple of swap size (%ld)\r\n",
  ...
  for (i = 0U; i < SFU_NB_MAX_DWL_AREA; i++)
  {
    if (SlotStartAdd[SLOT_DWL_1 + i] != 0U)
    {
      ...
      {
        TRACE("\r\n= [FWIMG] SLOT_DWL_%ld size (%ld) must be a multiple of swap size (%ld)\r\n",
  ...
  for (i = 0U; i < SFU_NB_MAX_DWL_AREA; i++)
  {
    if (SlotStartAdd[SLOT_DWL_1 + i] != 0U)
    {
      if (((int32_t)(SFU_IMG_CHUNK_SIZE - (TRAILER_INDEX(SLOT_DWL_1 + i) * sizeof(SFU_LL_FLASH_write_t)))) < 0)
      {
        ...
        TRACE("\r\n= [FWIMG] %ld bytes required for the swap metadata of SLOT_DWL_%ld is too much\r\n",
  ...
  {
    e_ret_status = SFU_IMG_INIT_SWAP_SETTINGS_ERROR;
    TRACE("\r\n= [FWIMG] The swap procedure uses chunks of %ld bytes but the swap size (%ld) is not a multiple\r\n",
  ...
  {
    e_ret_status = SFU_IMG_INIT_SWAP_SETTINGS_ERROR;
    TRACE("\r\n= [FWIMG] The swap procedure uses chunks of %ld bytes but the firmware start offset is %ld bytes\r\n",
  ...
  if (!IS_ALIGNED(SlotStartAdd[SLOT_SWAP]))
  {
    ...
    TRACE("\r\n= [FWIMG] swap (%lx) is not properly aligned\r\n",
...
SFU_ErrorStatus SFU_IMG_UpdateImageState(uint32_t SlotNumber)
{
  ...
  if (e_ret_status_se == SE_SUCCESS)
  {
    if (image_state == FWIMG_STATE_NEW)
    {
      ...
      {
#if defined(SFU_VERBOSE_DEBUG_MODE)
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%ld state = %ld failed to switch in FWIMG_STATE_SELFTEST", SlotNumber,
              image_state);
#endif /* (SFU_VERBOSE_DEBUG_MODE)     */
        e_ret_status = SFU_ERROR;
      }
