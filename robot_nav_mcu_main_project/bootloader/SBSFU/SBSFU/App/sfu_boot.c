...
#include "flashdb.h"
extern unsigned char y_update_flag;
...
static void SFU_BOOT_SM_CheckNewFwToDownload(void)
{
  ...
  if (initialDeviceStatusCheck == 1U)
  {
    ...
    //if (0U != BUTTON_PUSHED())
    if(HAL_GPIO_ReadPin(MX_BUTTON_PWR_GPIO_Port, MX_BUTTON_PWR_Pin) == GPIO_PIN_RESET || y_update_flag)
    {
...
static void SFU_BOOT_SM_CheckUserFwStatus(void)
{
  ...
  switch (e_PendingInstallStatus)
  {
    case SFU_IMG_FWIMAGE_ROLLBACK:
      ...
      TRACE("\r\n\t  Installation not validated: rollback procedure initiated (SLOT_ACTIVE_%ld / SLOT_DWL_%ld)",
    ...
    case SFU_IMG_FWUPDATE_STOPPED:
      ...
      TRACE("\r\n\t  Installation Failed: resume installation procedure initiated (SLOT_ACTIVE_%ld / SLOT_DWL_%ld)",
    ...
    case SFU_IMG_FWIMAGE_TO_INSTALL:
       ...
       */
      TRACE("\r\n\t  New Fw to be installed from slot SLOT_DWL_%ld", m_DwlSlotToInstall - SLOT_DWL_1 + 1U);
    ...
    case SFU_IMG_NO_FWUPDATE:
      ...
      if (MASTER_SLOT != 0xFFU)
      {
        if (SFU_SUCCESS == SFU_IMG_DetectFW(MASTER_SLOT))
        {
          ...
          TRACE("\r\n\t  A FW is detected in the slot SLOT_ACTIVE_%ld", m_ActiveSlotToExecute);
        ...
        for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
        {
          if (SlotStartAdd[SLOT_ACTIVE_1 + i] != 0U)       /* Slot configured ? */
          {
            if (SFU_IMG_VerifyEmptyActiveSlot(SLOT_ACTIVE_1 + i) != SFU_SUCCESS)
            {
               ...
               */
              TRACE("\r\n\t  Slot SLOT_ACTIVE_%ld not empty : erasing ...", SLOT_ACTIVE_1 + i);
...
static void SFU_BOOT_SM_DownloadNewUserFw(void)
{
  ...
  {
#if defined(SFU_VERBOSE_DEBUG_MODE)
    TRACE("\r\n\t  FwSize=%ld | PartialFwSize=%ld | PartialFwOffset=%ld | %ld bytes received",
      ...
      if (SFU_SUCCESS == SFU_IMG_DetectFW(SLOT_ACTIVE_1 + i))
      {
        ...
        if (SFU_SUCCESS == e_ret_status)
        {
          ...
          if (SFU_SUCCESS == e_ret_status)
          {
            ...
            {
#if defined(SFU_VERBOSE_DEBUG_MODE)
              TRACE("\r\n\t  Unexpected code beyond FW image in slot SLOT_ACTIVE_%ld", SLOT_ACTIVE_1 + i);
#endif /* SFU_VERBOSE_DEBUG_MODE */
            }
          }
          else
          {
#if defined(SFU_VERBOSE_DEBUG_MODE)
            TRACE("\r\n\t  Firmware signature verification failure in slot SLOT_ACTIVE_%ld", SLOT_ACTIVE_1 + i);
#endif /* SFU_VERBOSE_DEBUG_MODE */
          }
        }
        else
        {
#if defined(SFU_VERBOSE_DEBUG_MODE)
          TRACE("\r\n\t  Header signature verification failure in slot SLOT_ACTIVE_%ld", SLOT_ACTIVE_1 + i);
#endif /* SFU_VERBOSE_DEBUG_MODE */
        }
        /*
        ...
        if (SFU_SUCCESS != e_ret_status)
        {
           ...
           */
#if defined(SFU_VERBOSE_DEBUG_MODE)
          TRACE("\r\n\t  Erasing slot SLOT_ACTIVE_%ld", SLOT_ACTIVE_1 + i);
#endif /* SFU_VERBOSE_DEBUG_MODE */
          (void)SFU_IMG_InvalidateCurrentFirmware(SLOT_ACTIVE_1 + i); /* If this fails we continue anyhow */
        }
        else
...
static void SFU_BOOT_SM_ExecuteUserFw(void)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    if (SE_LockRestrictServices(&e_se_status) == SE_SUCCESS)
    {
      ...
      TRACE("\r\n=         SFU_IMG_LaunchActiveImg(SLOT_ACTIVE_%ld) failure!", m_ActiveSlotToExecute);
#endif /* SFU_VERBOSE_DEBUG_MODE */
      while (1 == 1)
      {
        ;    /* wait for watchdog */
...
static SFU_ErrorStatus  SFU_BOOT_ManageResetSources(void)
{
  ...
  /* 保存唤醒源标志 */
  fdb_kv_set_blob(&kvdb, "wakeup_src", fdb_blob_make(&blob, (uint8_t *)&e_wakeup_source_id, sizeof(e_wakeup_source_id)));
