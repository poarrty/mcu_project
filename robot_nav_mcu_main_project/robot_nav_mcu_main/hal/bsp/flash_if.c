...
/**
 * @brief  Depending on start address, this function will call internal or
 * external (OSPI/QSPI) flash driver
...
/**
 * @brief  Depending on destination address, this function will call internal or
 * external (OSPI/QSPI) flash driver
 ...
 * @brief  Depending on destination address, this function will call internal or
 * external (OSPI/QSPI) flash driver
 * @brief  Depending on source address, this function will call internal or
 * external (OSPI/QSPI) flash driver
...
/**
 * @brief  This function does an erase of n (depends on Length) sectors in user
 * flash area
    ...
    if (e_ret_status == HAL_OK) {
        ...
        if (HAL_FLASH_Unlock() == HAL_OK) {
            ...
            do {
                ...
                // HAL_WWDG_Refresh(&hwwdg);    /* sulikang add */
                ...
                /* Refresh Watchdog */
                // WRITE_REG(IWDG->KR, IWDG_KEY_RELOAD);         /* sulikang
                // remove
                // */ HAL_WWDG_Refresh(&hwwdg);    /* sulikang add */
            } while (nb_sectors > 0);
 ...
 * @param  pDestination: Start address for target location. It has to be 1 byte
 * aligned.
 * @param  pSource: pointer on buffer with data to write
