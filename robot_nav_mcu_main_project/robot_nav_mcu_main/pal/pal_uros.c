...
#define RCCHECK(fn)                                                                                 \
    {                                                                                               \
        ...
        if ((temp_rc != RCL_RET_OK)) {                                                              \
            log_w("\r\nUROS create something failed on line %d: %d.\r\n", __LINE__, (int) temp_rc); \
...
#define RCDESTORYCHECK(fn)                                                              \
    {                                                                                   \
        rcl_ret_t temp_rc = fn;                                                         \
        if ((temp_rc != RCL_RET_OK)) {                                                  \
            log_w("Failed status on line %d: %d. Continuing", __LINE__, (int) temp_rc); \
        } else {                                                                        \
            log_d("destory on line : %d finish", __LINE__);                             \
        }                                                                               \
    }
...
#define UROS_SYNC_OK_PERIOD_MS  (1 * 60 * 1000)
#define UROS_SYNC_ERR_PERIOD_MS (1 * 1000)

bool              is_pal_uros_stop  = false;
volatile uint16_t uros_remain_count = 0;

uint32_t        uros_ping_ts      = 0;
uint32_t        uros_sync_ts      = 0;
uint32_t        uros_heartbeat_ts = 0;
extern uint32_t uros_comm_check_ts;
...
void message_publish(ppublisher ptpublisher) {
    ...
    if (ptpublisher->msgqueue == OVERWRITE) {
        if (IS_IRQ()) {
            ...
            xQueueOverwriteFromISR((QueueHandle_t) ptpublisher->mq_id, ptpublisher->ros_message, &yield);

            if (yield == pdTRUE) {
                portYIELD_FROM_ISR(yield);
            }
        } else {
            xQueueOverwrite((QueueHandle_t) ptpublisher->mq_id, ptpublisher->ros_message);
        }
    } else {
...
void pal_uros_run(void *argument) {
    rmw_ret_t ping_ret = RMW_RET_ERROR;
    is_pal_uros_stop   = false;
    ...
    /* uros main loop ********************************************************/
    while (1) {
        while (is_pal_uros_stop) {
            osDelay(1000);
        }

        if (fal_pmu_get_sta() == PMU_STA_STARTING || fal_pmu_get_sta() == PMU_STA_RUNNING) {
            ...
            if (RMW_RET_OK == ping_ret) {
                if (is_uros_init_successful) {
                    /* 心跳发送检测 agent 会导致本线程卡在心跳发布函数，导致通信异常，但又检测不出来" */
                    /*
                    ...
                    */
                    ...
                    /* 还是可能卡在别的地方执行不到这里，转到 task_default_run 线程进行管理 */
                    /*
                    if (osKernelGetTickCount() >= uros_comm_check_ts) {
                        if (sys_exc_get(EXC35_NVG_SOC_COM_TO)) {
                            log_e("RK3399 speed cmd timeout, set uros ping flag to error.");

                            ping_ret = RMW_RET_ERROR;
                        }

                        uros_comm_check_ts = osKernelGetTickCount() + 10 * 1000;
                    }
                    */
                    ...
                    log_i("pal_uros_create_entities.");
                ...
                if (osKernelGetTickCount() >= uros_sync_ts) {
                    uros_remain_count++;
                    if (pal_uros_sync() == 0) {
                        uros_sync_ts = osKernelGetTickCount() + UROS_SYNC_OK_PERIOD_MS;
                    } else {
                        uros_sync_ts = osKernelGetTickCount() + UROS_SYNC_ERR_PERIOD_MS;
                    }
                    uros_remain_count--;
                }
            } else {
                if (is_uros_init_successful) {
                    ...
                    log_i("pal_uros_destroy_entities finish");
                }
...
int pal_uros_init(void) {
    ...
    rmw_uros_set_custom_transport(true, (void *) &huart1, freertos_serial_open, freertos_serial_close, freertos_serial_write,
    ...
    if (!rcutils_set_default_allocator(&freeRTOS_allocator)) {
        log_e("pal_uros_init error: set default allocators.");
        ret = 1;
    } else {
        log_i("pal_uros_init ok.");
        ret = 0;
    }
    ...
    /*
    publisher_init(&g_nav_heartbeat, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt8), "/nav_heartbeat", &nav_heartbeat, DEFAULT, OVERWRITE,
...
void pal_uros_create_entities(void) {
    ...
    while (1) {
        if (rclc_support_init(&support, 0, NULL, &allocator) == RCL_RET_OK) {
            log_i("uros agent connect ok.");
            break;
        } else {
            log_i("uros agent connect error, retry...");
        }
    ...
    RCCHECK(rclc_node_init_default(&node, "MCU_407", "", &support));

    /* publisher **************************************************************/
    while (p_pubtmp) {
        if (p_pubtmp->qos == BEST) {
            RCCHECK(rclc_publisher_init_best_effort(&p_pubtmp->publisher_t, &node, p_pubtmp->type_support, p_pubtmp->topic_name));
        } else if (p_pubtmp->qos == DEFAULT) {
            RCCHECK(rclc_publisher_init_default(&p_pubtmp->publisher_t, &node, p_pubtmp->type_support, p_pubtmp->topic_name));
        } else {
    ...
    log_i("publisher_register %ld , the max is %ld", publisher_register_count, RMW_UXRCE_MAX_PUBLISHERS);

    /* subscriber *************************************************************/
    while (p_subtmp) {
        if (p_subtmp->qos == BEST) {
            RCCHECK(rclc_subscription_init_best_effort(&p_subtmp->subscrption_t, &node, p_subtmp->type_support, p_subtmp->topic_name));
        } else if (p_subtmp->qos == DEFAULT) {
            RCCHECK(rclc_subscription_init_default(&p_subtmp->subscrption_t, &node, p_subtmp->type_support, p_subtmp->topic_name));
        } else {
    ...
    log_i("subscrption_register %ld , the max is %ld", subscrption_register_count, RMW_UXRCE_MAX_SUBSCRIPTIONS);

    /* service *************************************************************/
    while (p_srvtmp) {
        if (p_srvtmp->qos == BEST) {
            RCCHECK(rclc_service_init_best_effort(&p_srvtmp->service_t, &node, p_srvtmp->type_support, p_srvtmp->service_name));
        } else if (p_srvtmp->qos == DEFAULT) {
            RCCHECK(rclc_service_init_default(&p_srvtmp->service_t, &node, p_srvtmp->type_support, p_srvtmp->service_name));
        } else {
    ...
    log_i("service_register %ld , the max is %ld", service_register_count, RMW_UXRCE_MAX_SERVICES);

    /* executor **************************************************************/
    // Create a executor
    //服务加订阅
    RCCHECK(rclc_executor_init(&executor, &support.context, service_register_count + publisher_register_count, &allocator));

    /* add subcriber to executor *********************************************/
    p_subtmp = g_subscrption;

    while (p_subtmp) {
        RCCHECK(rclc_executor_add_subscription(&executor, &p_subtmp->subscrption_t, p_subtmp->ros_message, p_subtmp->callback, ON_NEW_DATA));
    ...
    while (p_srvtmp) {
        if (p_srvtmp->callback) {
            RCCHECK(
                rclc_executor_add_service(&executor, &p_srvtmp->service_t, p_srvtmp->request_msg, p_srvtmp->response_msg, p_srvtmp->callback));
        }
    ...
    if (is_uros_rclc_error_happened) {
        log_i("pal_uros_create_entities failed, destroy_entities..");
        pal_uros_destroy_entities();
        is_uros_init_successful = false;

    } else {
        log_i("pal_uros_create_entities success.");
...
bool pal_uros_stop(void) {
    is_uros_init_successful = false;
    is_pal_uros_stop        = true;

    if (uros_remain_count) {
        log_w("uros_remain_count :%d", uros_remain_count);
        return false;
    } else {
        return true;
    }
...
uint8_t pal_uros_sync(void) {
    ...
    if (rmw_uros_sync_session(100) == RMW_RET_OK) {
        ...
        ts.tv_sec  = (int32_t)(nanos / 1000000000) + 8 * 3600;  ///< 北京时间时区：东八区
        ...
        log_i(
            ...
            "%02d:%02d:%02d.%03d",
            time_utc->tm_year + 1900, time_utc->tm_mon + 1, time_utc->tm_mday, time_utc->tm_hour, time_utc->tm_min, time_utc->tm_sec,
            ts.tv_nsec / 1000000);
...
void pal_uros_print_ts(void) {
    ...
    log_i("pal_uros_print_ts: %d-%d-%d %02d:%02d:%02d.%03d", time_utc->tm_year + 1900, time_utc->tm_mon + 1, time_utc->tm_mday,
          time_utc->tm_hour, time_utc->tm_min, time_utc->tm_sec, ts.tv_nsec / 1000000);
}
