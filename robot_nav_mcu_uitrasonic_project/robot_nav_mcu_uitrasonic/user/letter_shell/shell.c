...
static const char *shellText[] = {
    ...
    [SHELL_TEXT_INFO] =
        ...
        "\033[32m| |   / _ \\ __| __/ _ \\ '__| / __| '_ \\ / _ \\ | "
        "|\033[0m\r\n"
        "\033[32m| |__|  __/ |_| ||  __/ |    \\__ \\ | | |  __/ | |\033[0m\r\n"
        "\033[32m|_____\\___|\\__|\\__\\___|_|    |___/_| "
        "|_|\\___|_|_|\033[0m\r\n"
        "\r\n"
        "\033[32mBuild:       "__DATE__
        " "__TIME__
        "\033[0m\r\n"
        "\033[32mVersion:     " SHELL_VERSION
        "\033[0m\r\n"
...
void shellInit(Shell *shell, char *buffer, unsigned short size) {
    ...
    shell->commandList.base =
        (ShellCommand *) (__section_begin("shellCommand"));
    shell->commandList.count =
        ((unsigned int) (__section_end("shellCommand")) -
...
#error not supported compiler, please use command table mode
    ...
    shellSetUser(shell, shellSeekCommand(shell, SHELL_DEFAULT_USER,
                                         shell->commandList.base, 0));
...
static void shellWriteCommandLine(Shell *shell, unsigned char newline) {
    ...
    if (shell->status.isChecked) {
        ...
        shellWriteString(
            shell, shell->info.path ? shell->info.path : "\033[32m/\033[0m");
        shellWriteString(shell, "\033[32m$ \033[0m");
    } else {
...
static const char *shellGetCommandName(ShellCommand *command) {
    ...
    for (unsigned char i = 0; i < 9; i++) { buffer[i] = '0'; }
...
void shellListItem(Shell *shell, ShellCommand *item) {
    ...
    do { shellWriteByte(shell, ' '); } while (--spaceLength);
    ...
    for (signed char i = 7; i >= 0; i--) {
        shellWriteByte(shell,
                       item->attr.attrs.permission & (1 << i) ? 'x' : '-');
    }
#endif
    shellWriteString(shell, "  ");
...
void shellDeleteCommandLine(Shell *shell, unsigned char length) {
    while (length--) { shellWriteString(shell, "\b \b"); }
}
...
void shellInsertByte(Shell *shell, char data) {
    ...
    if (shell->parser.cursor == shell->parser.length) {
        ...
        for (short i = shell->parser.length - shell->parser.cursor; i > 0;
             i--) {
        ...
        shell->parser.buffer[++shell->parser.length] = 0;
        for (short i = shell->parser.cursor - 1; i < shell->parser.length;
             i++) {
...
void shellDeleteByte(Shell *shell, signed char direction) {
    ...
    if (shell->parser.cursor == shell->parser.length && direction == 1) {
        ...
        for (short i = offset; i < shell->parser.length - shell->parser.cursor;
             i++) {
            shell->parser.buffer[shell->parser.cursor + i - 1] =
                shell->parser.buffer[shell->parser.cursor + i];
        }
        ...
        shellWriteByte(shell, ' ');
        for (short i = shell->parser.length - shell->parser.cursor + 1; i > 0;
             i--) {
            shellWriteByte(shell, '\b');
        }
    }
...
static void shellParserParam(Shell *shell) {
    ...
    for (unsigned short i = 0; i < shell->parser.length; i++) {
        if (quotes != 0 ||
            (shell->parser.buffer[i] != ' ' && shell->parser.buffer[i] != 0)) {
...
int shellGetVarValue(Shell *shell, ShellCommand *command) {
    ...
    switch (command->attr.attrs.type) {
        ...
        case SHELL_TYPE_VAR_NODE:
            value =
                ((ShellNodeVarAttr *) command->data.var.value)->get
                    ? ((ShellNodeVarAttr *) command->data.var.value)
                          ->get(((ShellNodeVarAttr *) command->data.var.value)
                                    ->var)
                    : 0;
            break;
        default:
...
int shellSetVarValue(Shell *shell, ShellCommand *command, int value) {
    if (command->attr.attrs.readOnly) {
        shellWriteString(shell,
                         shellText[SHELL_TEXT_VAR_READ_ONLY_CANNOT_MODIFY]);
    } else {
        switch (command->attr.attrs.type) {
            case SHELL_TYPE_VAR_INT:
            ...
            case SHELL_TYPE_VAR_CHAR:
                *((char *) (command->data.var.value)) = value;
                break;
            case SHELL_TYPE_VAR_STRING:
                shellStringCopy(((char *) (command->data.var.value)),
                                (char *) value);
                break;
            case SHELL_TYPE_VAR_POINT:
                shellWriteString(shell,
                                 shellText[SHELL_TEXT_POINT_CANNOT_MODIFY]);
                break;
            case SHELL_TYPE_VAR_NODE:
                if (((ShellNodeVarAttr *) command->data.var.value)->set) {
                    if (((ShellNodeVarAttr *) command->data.var.value)->var) {
                        ((ShellNodeVarAttr *) command->data.var.value)
                            ->set(((ShellNodeVarAttr *) command->data.var.value)
                                      ->var,
                                  value);
                    } else {
                        ((ShellNodeVarAttr *) command->data.var.value)
...
static int shellShowVar(Shell *shell, ShellCommand *command) {
    ...
    switch (command->attr.attrs.type) {
        ...
        default:
            ...
            for (short i = 0; i < 11; i++) { buffer[i] = '0'; }
...
int shellSetVar(char *name, int value) {
    ...
    ShellCommand *command =
        shellSeekCommand(shell, name, shell->commandList.base, 0);
    if (!command) {
...
unsigned int shellRunCommand(Shell *shell, ShellCommand *command) {
    ...
    if (command->attr.attrs.type == SHELL_TYPE_CMD_MAIN) {
        ...
        returnValue = shellExtRun(shell, command, shell->parser.paramCount,
                                  shell->parser.param);
        if (!command->attr.attrs.disableReturn) {
            shellWriteReturnValue(shell, returnValue);
        }
...
static void shellCheckPassword(Shell *shell) {
    if (strcmp(shell->parser.buffer, shell->info.user->data.user.password) ==
        0) {
        shell->status.isChecked = 1;
#if SHELL_SHOW_INFO == 1
...
static void shellHistoryAdd(Shell *shell) {
    ...
    if (shell->history.number > 0 &&
        strcmp(shell->history
                   .item[(shell->history.record == 0 ? SHELL_HISTORY_MAX_NUMBER
...
static void shellHistory(Shell *shell, signed char dir) {
    ...
    if (shell->history.offset == 0) {
        ...
        if ((shell->parser.length = shellStringCopy(
                 shell->parser.buffer,
                 shell->history
                     .item[(shell->history.record + SHELL_HISTORY_MAX_NUMBER +
                            shell->history.offset) %
...
void shellExec(Shell *shell) {
    ...
    if (shell->status.isChecked) {
        ...
        ShellCommand *command = shellSeekCommand(shell, shell->parser.param[0],
                                                 shell->commandList.base, 0);
        if (command != NULL) {
...
void shellTab(Shell *shell) {
    ...
    if (SHELL_GET_TICK()) {
        if (matchNum == 1 && shell->status.tabFlag &&
            SHELL_GET_TICK() - shell->info.activeTime <
...
void shellHelp(int argc, char *argv[]) {
    ...
    if (argc == 1) {
        ...
        ShellCommand *command =
            shellSeekCommand(shell, argv[1], shell->commandList.base, 0);
        if (command) {
...
void shellHandler(Shell *shell, char data) {
    ...
    if (shell->info.user->data.user.password &&
        strlen(shell->info.user->data.user.password) != 0 && SHELL_GET_TICK()) {
...
int shellRun(Shell *shell, const char *cmd) {
    ...
    if (strlen(cmd) > shell->parser.bufferSize - 1) {
        ...
        shell->parser.length =
            shellStringCopy(shell->parser.buffer, (char *) cmd);
        shellExec(shell);
