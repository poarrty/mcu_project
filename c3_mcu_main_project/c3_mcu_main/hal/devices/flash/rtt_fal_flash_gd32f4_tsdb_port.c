...
/* Base address of the GD32F4 Flash sectors Bank 2 */
#define ADDR_FLASH_SECTOR_24     ((uint32_t)0x08200000) /* Base @ of Sector 12, 256 Kbytes */
#define ADDR_FLASH_SECTOR_25     ((uint32_t)0x08240000) /* Base @ of Sector 13, 256 Kbytes */
#define ADDR_FLASH_SECTOR_26     ((uint32_t)0x08280000) /* Base @ of Sector 14, 256 Kbytes */
#define ADDR_FLASH_SECTOR_27     ((uint32_t)0x082C0000) /* Base @ of Sector 15, 256 Kbytes */
...
static uint32_t stm32_get_sector(uint32_t addr)
{
    ...
    else if((addr < ADDR_FLASH_SECTOR_24) && (addr >= ADDR_FLASH_SECTOR_23))
    {
        sector = FLASH_SECTOR_23;
    }
    else if((addr < ADDR_FLASH_SECTOR_25) && (addr >= ADDR_FLASH_SECTOR_24))
    {
        sector = FLASH_SECTOR_24;
    }
    else if((addr < ADDR_FLASH_SECTOR_26) && (addr >= ADDR_FLASH_SECTOR_25))
    {
        sector = FLASH_SECTOR_25;
    }
    else if((addr < ADDR_FLASH_SECTOR_27) && (addr >= ADDR_FLASH_SECTOR_26))
    {
...
{
    size_t i;
    long addr = stm32_onchip_tsdb_flash.addr + offset;
...
const struct fal_flash_dev stm32_onchip_tsdb_flash =
{
    .name       = "stm32_onchip_tsdb",
    .addr       = 0x08000000,
    .len        = 3076*1024,
    .blk_size   = 256*1024,
