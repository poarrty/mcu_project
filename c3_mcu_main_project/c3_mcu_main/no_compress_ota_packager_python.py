...
def gethead(filename,ori_obj,dst_obj):
    ...
    ###  ("fota_algo")
    fota_algo = 2#int(statinfo.st_ctime)
...
if __name__ == "__main__":
    key = b"C323456789ABCDEF0123456789ABCDEF"   # 32位
    iv = b"0123456789ABCDEF"    # 16位

    ### boot bin文件所在路径（只能有一个bin文件）
    bootbuildpath = "../c3_mcu_bootloader/build/"
    bootdirs = os.listdir(bootbuildpath)
    for bootfile in bootdirs:
        print (bootfile)
    bootfilename = bootbuildpath + bootfile
    ### boot的bin文件
    with open (bootfilename,"rb") as f:
        bootbin = f.read()
        print("boot文件长度：",len(bootbin))
    ### boot的剩余填充文件
    with open ("c3_mcu_main.elf","rb") as f:
        bootstr = f.read(128 * 1024 -  len(bootbin))
        print("剩余文件长度：",len(bootstr))
    ### app的剩余填充文件
    with open (bootfilename,"rb") as f:
        appstr = f.read(512 - 96)
    ### app bin文件所在路径（只能有一个bin文件）
    buildpath = "./build/"
    dirs = os.listdir(buildpath)
    for file in dirs:
        print (file)
    ...
    with open (filename,"rb") as f:
        appbin = f.read()
        print("加密压缩前文件长度：",len(appbin))
        ### gizp 压缩
        # tmp_gizp = gzip.compress(appbin,compresslevel=6)
        # tmp = gzipheader + tmp_gizp[10:]
        ### aes 加密
        res = AESUtil.encryt(appbin, key, iv)
        my_head = gethead(filename,appbin,res)
        print (my_head)
        print("加密压缩后文件长度：",len(res))
    ### 将加密后的文件写入rbl中    
    with open(buildpath + "ota_" + name + ".rbl","wb") as f:
        f.write(my_head)
        f.write(res)
    ### 生成带boot的bin文件   
    with open(buildpath + "ota_" + name + ".bin","wb") as f:
        f.write(bootbin)
        f.write(bootstr)
        f.write(my_head)
        f.write(appstr)
