 ...
 */
static crypto_status_t sha512_init(sha512_ctx_t *sha512_ctx,
                                   uint8_t *payload_addr, uint64_t payload_len,
                                   uint32_t is_sha384) {
 ...
 */
static crypto_status_t sha512_hash_factory(sha512_ctx_t *ctx,
                                           uint8_t data[128]) {
    uint32_t i = 0;
    uint64_t W[80];
    ...
    for (i = 0; i < 80;) {
        COMPRESS(v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], W[i], K[i]);
        i++;
        COMPRESS(v[7], v[0], v[1], v[2], v[3], v[4], v[5], v[6], W[i], K[i]);
        i++;
        COMPRESS(v[6], v[7], v[0], v[1], v[2], v[3], v[4], v[5], W[i], K[i]);
        i++;
        COMPRESS(v[5], v[6], v[7], v[0], v[1], v[2], v[3], v[4], W[i], K[i]);
        i++;
        COMPRESS(v[4], v[5], v[6], v[7], v[0], v[1], v[2], v[3], W[i], K[i]);
        i++;
        COMPRESS(v[3], v[4], v[5], v[6], v[7], v[0], v[1], v[2], W[i], K[i]);
        i++;
        COMPRESS(v[2], v[3], v[4], v[5], v[6], v[7], v[0], v[1], W[i], K[i]);
        i++;
        COMPRESS(v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[0], W[i], K[i]);
        i++;
    }
...
static crypto_status_t sha512_stage1(sha512_ctx_t *sha512_ctx) {
    ...
    while (sha512_ctx->payload_len >= 128) {
        ...
        SHA512_DEBUG("%x, %x\n", (uint32_t) sha512_ctx->payload_addr,
                     (uint32_t) sha512_ctx->payload_len);
    }
...
static crypto_status_t sha512_stage2(sha512_ctx_t *sha512_ctx,
    ...
    sha512_memcpy(sha512_ctx->payload_addr, temp_data_p,
                  sha512_ctx->payload_len);
 ...
 *          CRYPTO_SUCCESS if hash successed
 */
crypto_status_t easy_sha512(uint8_t *payload, uint64_t payload_len,
                            uint8_t hash[64]) {
 ...
 *          CRYPTO_SUCCESS if hash successed
 */
crypto_status_t easy_sha384(uint8_t *payload, uint64_t payload_len,
                            uint8_t hash[64]) {
