...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC),
                 show_task_state, show_task_state, show_task_state);
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC),
                 show_task_cpu, show_task_cpu, show_task_cpu);
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC),
                 show_heap_state, show_heap_state, show_heap_state);

uint8_t read_coredown(void) {
    fdb_kv_get_blob(
        &kvdb, "hardfault",
        fdb_blob_make(&blob, cStringBuffer, sizeof(cStringBuffer) + 1));
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC),
                 read_coredown, read_coredown, read_coredown);
...
void wwdgtimeout_coredown_save(void) {
    ...
    sprintf(cStringBuffer + strlen(cStringBuffer), "Last running task: %s",
            vTaskName());
    ...
    fdb_kv_set_blob(
        &kvdb, "wwdgtimeout",
        fdb_blob_make(&blob, cStringBuffer, strlen(cStringBuffer) + 1));
...
void hardfault_coredown_append(const char *format, ...) {
    ...
    if (flag == 0) {
        ...
        sprintf(cStringBuffer, "[Hardfault at %d-%d-%d %02d:%02d:%02d]\r\n",
                date.Year + 2000, date.Month, date.Date, time.Hours,
...
uint8_t sys_reboot_source(void) {
    printf("\r\n");
...
// SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), sys_reboot_source, sys_reboot_source,
// sys_reboot_source);
