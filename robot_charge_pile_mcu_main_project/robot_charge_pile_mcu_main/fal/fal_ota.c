...
#define APP_KEY "938983395d60d12117752588fe37616442444ef6"
...
void fal_ota_init(void) {
    subscrption_init(&g_discover,
                     ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt8),
                     "/ota/discover", &discover, BEST, fal_ota_discovered);

    publisher_init(
        &g_firmware_info,
        ROSIDL_GET_MSG_TYPE_SUPPORT(chassis_interfaces, msg, FirmwareInfo),
        "/ota/firmware_info", &firmware_info, BEST, OVERWRITE,
        sizeof(chassis_interfaces__msg__FirmwareInfo));

    publisher_init(
        &g_upgrade_progress,
        ROSIDL_GET_MSG_TYPE_SUPPORT(chassis_interfaces, msg, UpgradeProgess),
        "/ota_" APP_KEY "/upgrade_progress", &upgrade_progress, BEST, OVERWRITE,
        sizeof(chassis_interfaces__msg__UpgradeProgess));

    service_init(&g_upgrade,
                 ROSIDL_GET_SRV_TYPE_SUPPORT(chassis_interfaces, srv, Upgrade),
                 "/ota_" APP_KEY "/upgrade", &upgrade_req, &upgrade_res, BEST,
                 fal_upgrade);

    service_init(
        &g_get_updating_info,
        ROSIDL_GET_SRV_TYPE_SUPPORT(chassis_interfaces, srv, GetUpdatingInfo),
        "/ota_" APP_KEY "/get_updating_info", &get_updating_info_req,
        &get_updating_info_res, BEST, fal_get_updating_info);
...
int ota_write_file_temp(uint32_t addr, uint8_t *buffer, uint32_t count) {
    ...
    if (partition == NULL) {
        LOG_WARNING("Find partition (%s) failed!\n", "main_mcu_temp");
        return -1;
    }
...
void ota_erase_file_temp(void) {
    ...
    if (fal_partition_erase_all(partition) < 0) {
        LOG_WARNING("Partition (%s) erase failed!\n", partition->name);
        return;
    }

    LOG_DEBUG("Erase (%s) partition finish!\n", "main_mcu_temp");
}

static void fal_ota_check_update(void) {
    fdb_kv_get_blob(
        &kvdb, "kv_updating_info",
        fdb_blob_make(&blob, &kv_updating_info, sizeof(kv_updating_info)));
    fdb_kv_get_blob(
        &kvdb, "kv_firmware_info",
        fdb_blob_make(&blob, &kv_firmware_info, sizeof(kv_firmware_info)));

    /*版本不一致视为更新成功*/
    if (strcmp(kv_firmware_info.version, SOFTWARE_VERSION) != 0) {
        /*删除续传信息*/
        memset(&kv_updating_info, 0, sizeof(kv_updating_info));
        memset(kv_updating_info.total_md5, 0,
               sizeof(kv_updating_info.total_md5));
        fdb_kv_set_blob(
            &kvdb, "kv_updating_info",
            fdb_blob_make(&blob, &kv_updating_info, sizeof(kv_updating_info)));
        /*OTA完成后，更新时间*/
        ...
        LOG_DEBUG("ota finish");
    }
}

void fal_ota_discovered(const void *msgin) {
    LOG_DEBUG("OTA discoverd");
    strcpy(kv_firmware_info.version, SOFTWARE_VERSION);
    LOG_DEBUG("kv_firmware_info.version :%s", kv_firmware_info.version);
    ...
    firmware_info.upgrade_service_name.size =
        strlen(firmware_info.upgrade_service_name.data);

    firmware_info.updating_info_service_name.data =
        "/ota_" APP_KEY "/get_updating_info";
    firmware_info.updating_info_service_name.size =
        strlen(firmware_info.updating_info_service_name.data);

    firmware_info.upgrade_progress_topic_name.data =
        "/ota_" APP_KEY "/upgrade_progress";
    firmware_info.upgrade_progress_topic_name.size =
        strlen(firmware_info.upgrade_progress_topic_name.data);
    /*接受到discover请求后，发送固件信息topic*/
    message_publish(&g_firmware_info);
}

void fal_upgrade(const void *req, void *res) {
    chassis_interfaces__srv__Upgrade_Request *req_in =
        (chassis_interfaces__srv__Upgrade_Request *) req;
    chassis_interfaces__srv__Upgrade_Response *res_in =
        (chassis_interfaces__srv__Upgrade_Response *) res;

    if (req_in->package_data.size != req_in->package_size) {
        LOG_WARNING("error message, package_data.size:%d, package_size:%ld",
                    req_in->package_data.size, req_in->package_size);
        res_in->success = 0;
        return;
    }

    if ((req_in->package_size >= req_in->total_size) ||
        (req_in->package_size > OTA_PACKSIZE)) {
        LOG_WARNING(
            "error message, package_size:%ld, total_size:%ld, ota_packsize:%d",
            req_in->package_size, req_in->total_size, OTA_PACKSIZE);
        res_in->success = 0;
        return;
    }
    ...
    if (memcmp(md5_cal_temp, req_in->package_md5.data, 32)) {
        LOG_WARNING("md5 cal :%s", md5_cal_temp);
        LOG_WARNING("md5 receive :%s", req_in->package_md5.data);
        res_in->success = 0;
    } else {
        /*认为第一次接受文件*/
        if (req_in->package_offset == 0) {
            ...
            memset(kv_updating_info.total_md5, 0,
                   sizeof(kv_updating_info.total_md5));
            memcpy(kv_updating_info.total_md5, req_in->total_md5.data,
                   req_in->total_md5.size);
            /*擦除备份区*/
            LOG_DEBUG("ota erase file temp");
            ota_erase_file_temp();
            kv_updating_info.buffered_size = 0;
        }

        /*更新已写入文件大小*/
        if (ota_write_file_temp(req_in->package_offset,
                                (uint8_t *) req_in->package_data.data,
                                req_in->package_size) < 0) {
            LOG_WARNING("flash write 0x%8lx error", req_in->package_offset);
            res_in->success = 0;
        } else {
            LOG_DEBUG("flash write 0x%8lx ok, package_size: %ld",
                      req_in->package_offset, req_in->package_size);
            LOG_DEBUG(
                "ota receive %ld percent",
                (kv_updating_info.buffered_size * 100 / req_in->total_size));
        ...
        if (kv_updating_info.buffered_size == req_in->total_size) {
            LOG_DEBUG("ota receive all %ld Byte ok", req_in->total_size);
            ...
            for (i = 0; i < req_in->total_size / OTA_PACKSIZE; i++) {
                ota_read_file_temp(i * OTA_PACKSIZE, file_temp_buff,
                                   OTA_PACKSIZE);
                MD5Update(&md5, file_temp_buff, OTA_PACKSIZE);
            }

            ota_read_file_temp(i * OTA_PACKSIZE, file_temp_buff,
                               req_in->total_size - i * OTA_PACKSIZE);
            MD5Update(&md5, file_temp_buff,
                      req_in->total_size - i * OTA_PACKSIZE);
            MD5Final(&md5, decrypt);
            ...
            if (memcmp(md5_cal_temp, req_in->total_md5.data, 32)) {
                LOG_WARNING("total md5 cal :%s", md5_cal_temp);
                LOG_WARNING("total md5 receive : %s", req_in->total_md5.data);
                res_in->success = 0;
            } else {
...
void fal_get_updating_info(const void *req, void *res) {
    ...
    chassis_interfaces__srv__GetUpdatingInfo_Response *res_in =
        (chassis_interfaces__srv__GetUpdatingInfo_Response *) res;
    ...
    ota_read_file_temp(i * OTA_PACKSIZE, file_temp_buff,
                       kv_updating_info.buffered_size - i * OTA_PACKSIZE);
    MD5Update(&md5, file_temp_buff,
              kv_updating_info.buffered_size - i * OTA_PACKSIZE);
    MD5Final(&md5, decrypt);

    memset(kv_updating_info.buffered_md5, 0,
           sizeof(kv_updating_info.buffered_md5));
    ...
    LOG_DEBUG("total_md5:%s", res_in->total_md5.data);
    LOG_DEBUG("buffered_md5:%s", res_in->buffered_md5.data);
    LOG_DEBUG("buffered_size:%ld", res_in->buffered_size);
}