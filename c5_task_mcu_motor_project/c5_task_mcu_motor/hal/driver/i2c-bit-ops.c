...
static rt_err_t SCL_H(struct rt_i2c_bit_ops *ops) {
    ...
    while (!GET_SCL(ops)) {
        if ((xTaskGetTickCount() - start) > ops->timeout) {
            return -RT_ETIMEOUT;
        }
    ...
    if (rt_tick_get() != start) {
        log_d("wait %ld tick for SCL line to go high", rt_tick_get() - start);
    }
...
static rt_size_t i2c_recv_bytes(struct rt_i2c_bus_device *bus, struct rt_i2c_msg *msg) {
    ...
    while (count > 0) {
        ...
        log_d("recieve bytes: 0x%02x, %s", val, (flags & RT_I2C_NO_READ_ACK) ? "(No ACK/NACK)" : (count ? "ACK" : "NACK"));
    ...
    for (i = 0; i < num; i++) {
        ...
        if (!(msg->flags & RT_I2C_NO_START)) {
            ...
            if ((ret != RT_EOK) && !ignore_nack) {
                log_d("receive NACK from device addr 0x%02x msg %d", msgs[i].addr, i);
                goto out;
            }
        }
...
const struct rt_i2c_bus_device_ops i2c_bit_bus_ops = {i2c_bit_xfer, RT_NULL, RT_NULL};
